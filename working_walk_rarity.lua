...
if BD<5 then TL.Text="BUY..";for attempt=1,2 do V:SendKeyEvent(true,Enum.KeyCode.E,false,game);for i=1,20 do if not BT or not BT.Parent then break end;local np=BT.PrimaryPart and BT.PrimaryPart.Position or BP;H:MoveTo(np);wait(0.1) end;V:SendKeyEvent(false,Enum.KeyCode.E,false,game);if not BT or not BT.Parent then break end;wait(0.1) end;IG[BT]=tick();if not BT or not BT.Parent then TL.Text="OK!" else TL.Text="MISS" end;wait(0.2);return end
