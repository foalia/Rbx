...
local function sendWH(msg) pcall(function() request({Url=WEBHOOK,Method="POST",Headers={["Content-Type"]="application/json"},Body=HS:JSONEncode({content="**["..L.Name.."]** "..msg})}) end) end
...
-- Anti-stuck: mundur kalau tidak bergerak
if SC>8 then H.Jump=true;H:MoveTo(R.Position-R.CFrame.LookVector*10);SC=0;wait(0.3);return else H:MoveTo(BP) end
-- Buy logic: dekat + cari ProximityPrompt atau tekan E
if BD<5 then TL.Text="BUY..";local bought=false
