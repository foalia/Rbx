...
local MIN,lastAFK,AFKi,lastWH=6,tick(),math.random(30,60),""
...
local function sendWH(msg) if msg==lastWH then return end;lastWH=msg;pcall(function() local data={content="**["..L.Name.."]** "..msg} request({Url=WEBHOOK,Method="POST",Headers={["Content-Type"]="application/json"},Body=HS:JSONEncode(data)}) end) end
...
if BD<6 then H:MoveTo(R.Position);TL.Text="BUY..";V:SendKeyEvent(true,Enum.KeyCode.E,false,game);wait(HD);V:SendKeyEvent(false,Enum.KeyCode.E,false,game);wait(0.5);if not BT or not BT.Parent then TL.Text="OK!";sendWH("Bought: "..BN.." ["..BRN.."]") end end
else TL.Text="...";SC=0;lastWH="" end end) end end end)
